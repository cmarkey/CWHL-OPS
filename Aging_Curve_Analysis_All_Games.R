...
#df <- CWHL_Reg_Player_Stats_OPS[CWHL_Reg_Player_Stats_OPS$'Player ID' == "Pierri_Jacquie",]

ggplot(data = df, aes(x = Age, y = OPS))+
  geom_line(size = 3, color = "#297d53") +
  geom_point(size = 5, color = "#297d53") +
  labs(title = "Aging Curve - Jacquie Pierri", y = "Offensive Point Shares", color = "Player")+
  theme_light()+
  theme(plot.title = element_text(hjust = .5, size = 36), axis.title = element_text(hjust = 0.5, size = 30), axis.text = element_text(size = 24))
...
avg_delta = data.frame("Age_Bucket" = c( "16/17", "17/18", "18/19", "19/20", "20/21", "21/22", "22/23", 
                                         "23/24", "24/25", "25/26", "26/27", "27/28", "28/29", "29/30", "30/31", 
                                         "31/32", "32/33", "33/34", "34/35", "35/36", "36/37", "37/38", "38/39", 
                                         "39/40", "40/41"), 
                        "Total_Players" = c( nrow(ops_1617), 
                                            nrow(ops_1718), nrow(ops_1819),
                                            nrow(ops_1920), nrow(ops_2021), 
                                            ...
                                            nrow(ops_3940), nrow(ops_4041)), 
                        "Avg_Delta_OPS" = c(avg_delta_1617,
                                            avg_delta_1718, avg_delta_1819,
                                            avg_delta_1920, avg_delta_2021,
                                            ...
                                            avg_delta_3940, avg_delta_4041))

avg_delta$Cumulative_Change <- cumsum(avg_delta$Avg_Delta_OPS)
avg_delta$Cumulative_Change <- avg_delta$Cumulative_Change - max(avg_delta$Cumulative_Change)

a <- ggplot(data = avg_delta, aes(x = Age_Bucket, y = Cumulative_Change, group =1))+
  geom_line(size = 3, color = "#297d53") +
  geom_point(size = 5, color = "#297d53") +
  labs(title = "Aging Curve - All Games", y = "Offensive Point Shares", x = "Age Bucket")+
  theme_light()+
